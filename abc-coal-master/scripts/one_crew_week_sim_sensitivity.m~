function [ cost, demurrage, crew ] = one_crew_week_sim_sensitivity( n, dt, tipple, engine_cost )
...
for i = 1:7
    arrivals(i,:) = sort(randi([int64(((i-1)*24) / dt), int64((15 + (i-1)*24) / dt)], 1, n) * dt);
end
...
arrivals(:,1) = 40000;
arrivals(5,1) = randi([int64((6 + (5-1)*24) / dt), int64((8 + (5-1)*24) / dt)], 1, 1) * dt;
...
trainCost = ones(1,n) * engine_cost * 3 * dt;
trainCost(1,1) = engine_cost * 5 * dt;
...
time = 0.0;
cost = zeros(1, int64(endTime / dt));
demurrage = zeros(1, int64(endTime / dt));
crew = zeros(1, int64(endTime / dt));
