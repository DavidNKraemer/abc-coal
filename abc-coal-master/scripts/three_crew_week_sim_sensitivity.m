function [ cost, demurrage, crew ] = three_crew_week_sim_sensitivity( n, dt, tipple, engine_cost )
...
crew = zeros(1, endTime / dt);interval = 1;
...
while (time < endTime / dt)
    ...
    if sum(loading)
        ...
        if tipple < 1 && sum(waiting)
            tipple = min(1.5, tipple + 3/4 * dt);  % Q4: change here suppose there exists a thrid crew
            crewFlag = 1;
            crewCost = 33000.0; % also crewCost changes when we have a third crew
